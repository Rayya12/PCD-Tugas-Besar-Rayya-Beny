...
def apply_histogram_specification(image: np.ndarray, histogram: list[int], format: str = "PNG") -> np.ndarray:
    ...
    if len(histogram) != 256:
        raise ValueError("histogram harus memiliki tepat 256 nilai")

    histogram_np = np.array(histogram, dtype=np.float64)

    result = np.zeros_like(image)
    for ch in range(3):
        result[:, :, ch] = _specify_channel(image[:, :, ch], histogram_np)
    return result


def _specify_channel(channel: np.ndarray, histogram: np.ndarray) -> np.ndarray:
    H, W = channel.shape
    total_pixels = H * W
    ...
    target_total = histogram.sum()
    if target_total == 0:
        raise ValueError("histogram tidak boleh semua nol")

    target_cdf = np.zeros(256, dtype=np.float64)
    target_cdf[0] = histogram[0]
    for i in range(1, 256):
        target_cdf[i] = target_cdf[i-1] + histogram[i]
